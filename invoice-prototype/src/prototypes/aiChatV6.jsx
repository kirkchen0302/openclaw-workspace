      ...
      {[0, 1, 2].map((d) => (
        <div
          ...
          style={{
            ...
            animation: `aichat6-bounce 1.2s infinite ${d * 0.2}s`,
          }}
        />
      ))}
    </div>
...
// ── Main component ──────────────────────────────────────────────────────
export default function AIChatV6({
  invoices,
  invoiceCount,
  totalAmount,
  monthlyTrend,
  ...
  function handleAuthChipTap(chip) {
    ...
    if (chip.key === "auth-yes") {
      ...
      // "先不用了" — show message with start CTA
      setPhase("declined");
      simulateBotReply([
        { type: "text", content: "沒問題！你隨時可以開始分析 👇" },
        { type: "start-cta" },
      ]);
    }
  }
  ...
  function buildHiddenResponse(data) {
    ...
    // Shocking headline — make it feel surprising
    const topName = top.name.length > 15 ? top.name.slice(0, 15) + "…" : top.name;
    const topAvg = Math.round(top.amount / Math.max(top.count, 1));
    parts.push({
      type: "text",
      content: `光是「${topName}」你就花了 $${fmt(top.amount)}——買了 ${top.count} 次，平均每次 $${fmt(topAvg)}。`,
    });
  ...
  function buildSaveResponse(data) {
    ...
    if (data.subscriptions && data.subscriptions.length > 0) {
      ...
      subBubble.push({
        type: "text",
        content: "📱 目前沒有偵測到訂閱服務的發票。\n\n但你可能有以下服務正在扣款，只是發票沒歸戶進來：",
      });
      subBubble.push({
        type: "datacard",
        title: "常見訂閱服務（確認是否有在使用）",
        rows: [
          { label: "🎬 Netflix / Disney+", value: "$170-390/月" },
          { label: "🎵 Spotify / KKBOX / Apple Music", value: "$149-170/月" },
          { label: "☁️ iCloud / Google One 雲端空間", value: "$30-300/月" },
          { label: "📺 YouTube Premium", value: "$179/月" },
          { label: "🛵 UberEats / foodpanda 會員", value: "$120-169/月" },
          { label: "🛒 酷澎 WOW / 蝦皮 VIP", value: "$59-99/月" },
        ],
      });
      subBubble.push({
        type: "text",
        content: "如果有在使用以上服務，建議歸戶載具讓發票進來，才能完整追蹤你的訂閱支出。",
      });
      subBubble.push({
        type: "cta",
        label: "📋 歸戶更多訂閱發票",
  ...
  // auth/loading/done/declined → no chips shown (auth uses dialog, declined has in-chat CTA)
  ...
  return (
    ...
    >
        ...
        <>
          ...
          <div
            style={{
              ...
              animation: "aichat6-slideUp 0.3s ease",
            }}
          >
      ...
      <style>{`
        @keyframes aichat6-slideUp {
          from { transform: translateY(100%); }
          to { transform: translateY(0); }
        }
        @keyframes aichat6-bounce {
