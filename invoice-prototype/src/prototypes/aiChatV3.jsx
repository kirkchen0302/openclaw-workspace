...
export default function AIChatV3({
  ...
  function buildWhereResponse(data) {
    ...
    // Audience tags
    if (data.audience && data.audience.tags && data.audience.tags.length > 0) {
      parts.push({
        type: "text",
        content: `根據你的消費模式，你是：${data.audience.tags.join("、")}`,
      });
    }
  ...
  function buildRepeatResponse(data) {
    ...
    if (data.subscriptions && data.subscriptions.length > 0) {
      ...
      parts.push({
        type: "text",
        content: `訂閱服務年度合計：$${fmt(annualSub)}/年`,
      });
    } else {
      parts.push({
        type: "text",
        content: "未偵測到訂閱服務。如果你有使用串流、外送等月費服務，可能尚未歸戶該載具。",
      });
      parts.push({
        type: "cta",
        label: "去手機設定檢查訂閱",
        primary: true,
        todoText: "檢查手機設定中的訂閱服務",
      });
    }

    // ── Section 2: Repeat Purchase Items ──────────────────────────────
      ...
      parts.push({
        ...
        rows: repeatItems.map((item) => {
          ...
          return {
            label: displayName,
            value: `${item.count} 次`,
            sub: `共 $${fmt(item.total)}，月均 $${fmt(monthly)}（在 ${item.shop}）`,
          };
        }),
      });
      parts.push({
    ...
    if (saves.length > 0) {
      ...
      if (data.smartBuy && data.smartBuy.length > 0) {
        parts.push({
          type: "datacard",
          title: "聰明採購",
          rows: data.smartBuy.map((s) => ({
            label: s.item,
            value: s.currentPrice && s.betterPrice
              ? `$${fmt(s.currentPrice)} → $${fmt(s.betterPrice)}`
              : "",
            sub: s.tip,
            valueColor: s.currentPrice && s.betterPrice ? T.success : undefined,
          })),
        });
      }

      // Total saveable with comparisons
      if (totalSaveable > 0) {
        parts.push({
          type: "text",
      ...
      parts.push({
        ...
        label: "記住省錢計畫",
        primary: true,
        todoText: "執行省錢計畫：" + saves.map((s) => s.action).join("、"),
      });
    }
